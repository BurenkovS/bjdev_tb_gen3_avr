...
#define _MICRO_MENU_H_
	...
	typedef const struct Menu_Item {
		...
		void (*RedrawCallback)(void); /**< Pointer to the optional menu-specific enter callback of this menu item */
		const char Text[16]; /**< Menu item text to pass to the menu display callback function */
		uint8_t NumericParam; /**< Menu item number for regular structure menu */
	} Menu_Item_t;
	 ...
	 *  \param[in] RedrawFunc  Function callback to redraw menu screen if needed(f.e. some external event), or \c NULL for no callback.
	 */
	#define MENU_ITEM(Name, Next, Previous, Parent, Child, SelectFunc, EnterFunc, RedrawFunc, Text, NumericParam) \
		...
		Menu_Item_t MENU_ITEM_STORAGE Name = {&Next, &Previous, &Parent, &Child, SelectFunc, EnterFunc, RedrawFunc, Text, NumericParam}
